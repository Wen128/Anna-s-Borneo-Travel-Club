...
// Initialize selectedAttraction as an empty object
...
// Function to add a new attraction (for POST request)
const addAttraction = () => {
...
const updateAttraction = () => {
  axios
    ...
    .then(() => {
      ...
      data.value[index] = editAttraction.value;
      showSidebar.value = false;
      console.log(editAttraction.value)
    })
    .catch((error) => {
...
const handleEdit = (attraction) => {
  ...
  selectedAttraction.value = toRaw(attraction.cellData).AttractionID; 
  editAttraction.value = data.value.find((attraction) => attraction.AttractionID == selectedAttraction.value);
  console.log("editattraction", editAttraction.value)
...
const handleDelete = (attraction) => {
  attractionToDelete.value = toRaw(attraction.cellData).AttractionID;
...
<template>
  <div>
    <div class="w-full flex justify-between">
      <h1 class="text-2xl font-bold text-blue-700">Attractions List</h1>
      <!-- Add Attraction Button -->
      <button @click="showSidebar = true, edit=false" class="btn btn-primary cursor-pointer mb-4 px-3 py-2 bg-blue-500 text-white rounded-md hover:bg-blue-600">Add Attraction</button>
  ...
  <path stroke-linecap="round" stroke-linejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" />
    ...
    <div class="sidebar" v-show="showSidebar">
      <div class="sidebar-content">
        <h3 class="mb-4">{{ edit ? 'Edit Attraction' : 'Add Attraction' }}</h3>
        <form @submit.prevent="edit ? updateAttraction() : addAttraction()">
...
<div v-show="showDeleteConfirm" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
  <div class="bg-white rounded-lg shadow-lg p-6 w-80">
    <h3 class="text-lg font-semibold text-gray-800 mb-4">Are you sure you want to delete this Attraction?</h3>
    <div class="flex justify-end space-x-4">
      <button @click="deleteAttraction" class="bg-red-600 text-white px-4 py-2 rounded-lg hover:bg-red-700 transition duration-200">
        Yes, Delete
      </button>
